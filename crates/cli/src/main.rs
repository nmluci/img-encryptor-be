...
fn format_img_format(format: ImageFormat) -> u8 {
   format as u8
}
...
fn decrypt_data(filepath: &PathBuf, outpath: &Option<PathBuf>, key: &String) {
   ...
   let pat_img = security::aes256_decrypt(pat_key, pat_data).expect("failed to decrypt pattern img");
...
fn main() {
   ...
   match &cli.mode {
      Some(Modes::Encrypt {
         ...
         output,
         thumbnail,
      }) => encrypt_data(filepath, thumbnail, output, key),
      Some(Modes::Decrypt {
         filepath,
         key,
         output,
